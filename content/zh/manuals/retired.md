---
title: 已弃用和已退役的 Docker 产品和功能
linkTitle: 已弃用的产品和功能
description: |
  探索已弃用和已退役的 Docker 功能、产品和开源项目，
  包括已转移工具和已归档项目的详细信息。
params:
  sidebar:
    group: Products
...
本文档概述了已弃用、已退役或已转移的 Docker 功能、产品和开源项目。
...
Docker, Inc. 不再为这些已弃用或已退役的功能提供支持。已转移给第三方的项目继续从其新维护者处接收更新。

### Docker Machine

Docker Machine 是一个用于在各种平台（包括虚拟机和云提供商）上配置和管理 Docker 主机的工具。它已不再维护，建议用户在支持的平台上直接使用 [Docker Desktop](/manuals/desktop/_index.md) 或 [Docker Engine](/manuals/engine/_index.md)。Machine 创建和配置主机的方法已被与 Docker Desktop 更紧密集成的更现代的工作流程所取代。

### Docker Toolbox

Docker Toolbox 用于无法运行 Docker Desktop 的旧系统。它将 Docker Machine、Docker Engine 和 Docker Compose 捆绑在一个安装程序中。Toolbox 已不再维护，在当前系统上实际上已被 [Docker Desktop](/manuals/desktop/_index.md) 取代。在较旧的文档或社区教程中偶尔会出现对 Docker Toolbox 的引用，但不建议用于新安装。

### Docker Cloud 集成

Docker 之前提供了与 Amazon 的 Elastic Container Service (ECS) 和 Azure Container Instances (ACI) 的集成，以简化容器工作流程。这些集成已被弃用，用户现在应该依赖原生云工具或第三方解决方案来管理其工作负载。向平台特定或通用编排工具的转变减少了对专用 Docker Cloud 集成的需求。

您仍然可以在 [Compose CLI 仓库](https://github.com/docker-archive/compose-cli/tree/main/docs)中查看这些集成的相关文档。

### Docker Enterprise Edition

Docker Enterprise Edition (EE) 是 Docker 用于部署和管理大规模容器环境的商业平台。它于 2019 年被 Mirantis 收购，寻找企业级功能的用户现在可以探索 Mirantis Kubernetes Engine 或 Mirantis 提供的其他产品。Docker EE 中的许多技术和功能已被整合到 Mirantis 产品线中。
...
Docker Data Center (DDC) 是一个总称，包括 Docker Universal Control Plane (UCP) 和 Docker Trusted Registry (DTR)。这些组件为企业环境中的容器管理、安全性和注册表服务提供了完整的解决方案。在 Docker Enterprise 收购后，它们现在归 Mirantis 所有。仍然遇到 DDC、UCP 或 DTR 引用的用户应参阅 Mirantis 的文档以获取现代等效产品的指导。

### Dev Environments

Dev Environments 是 Docker Desktop 中引入的一项功能，允许开发人员快速启动开发环境。它已被弃用并从 Docker Desktop 4.42 及更高版本中移除。可以通过 Docker Compose 或创建针对特定项目需求定制的自定义配置来实现类似的工作流程。

## 开源项目

几个最初由 Docker 维护的开源项目已被归档、停止或转移给其他维护者或组织。
...
Docker Compose v1（`docker-compose`）是一个用于定义多容器应用程序的基于 Python 的工具，已被 Compose v2（`docker compose`）取代，后者使用 Go 编写并与 Docker CLI 集成。Compose v1 已不再维护，用户应迁移到 Compose v2。
...
SwarmKit 通过为容器部署提供编排功能来支持 Docker Swarm 模式。虽然 Swarm 模式仍然可用，但开发已放缓，转而支持基于 Kubernetes 的解决方案。评估容器编排选项的用户应调查 SwarmKit 是否满足现代工作负载的要求。
