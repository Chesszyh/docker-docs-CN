...
为了减少推送/拉取时间和镜像大小，Compose 应用程序的一个常见做法是让服务尽可能多地共享基础层。通常会为所有服务选择相同的操作系统基础镜像。但您还可以更进一步，当您的镜像共享相同的系统软件包时，共享镜像层。需要解决的挑战是避免在所有服务中重复完全相同的 Dockerfile 指令。

为了说明，本页假设您希望所有服务都使用 `alpine` 基础镜像构建，并安装系统软件包 `openssl`。

## 多阶段 Dockerfile

推荐的方法是将共享声明分组到单个 Dockerfile 中，并使用多阶段功能，以便服务镜像从此共享声明构建。
...
一种流行的模式是在另一个服务中复用某个服务镜像作为基础镜像。由于 Compose 不解析 Dockerfile，它无法自动检测服务之间的这种依赖关系以正确排序构建执行。
...
旧版 Docker Compose v1 过去按顺序构建镜像，这使得这种模式可以直接使用。Compose v2 使用 BuildKit 来优化构建并并行构建镜像，因此需要显式声明。
...
通过 `additional_contexts` 属性，您可以引用由另一个服务构建的镜像，而无需显式命名它：
...
Bake 也可以通过编辑 `$HOME/.docker/config.json` 配置文件来设置为默认构建器：
```json
{
  ...
  "plugins": {
...
- [Docker Compose build 参考](/reference/cli/docker/compose/build.md)
