...
> [!IMPORTANT]: 从此版本开始，`docker service create`、`docker service update`、`docker service scale` 和 `docker service rollback` 默认使用非分离模式，使用 `--detach` 保持旧行为。

### 构建器

* 将上传的构建上下文中的 uid/gid 重置为 0 以便与其他客户端共享构建缓存 [docker/cli#513](https://github.com/docker/cli/pull/513)
+ 添加对没有任何子路径的 `ADD` url 的支持 [moby/moby#34217](https://github.com/moby/moby/pull/34217)
...
* 对 `docker service` 命令使用非分离模式作为默认值 [docker/cli#525](https://github.com/docker/cli/pull/525)
...
* 更改 `docker container stats` 的默认输出格式以显示 `CONTAINER ID` 和 `NAME` [docker/cli#565](https://github.com/docker/cli/pull/565)
+ 为 `docker container stats` 添加 `--no-trunc` 标志 [docker/cli#565](https://github.com/docker/cli/pull/565)
+ 添加实验性 `docker trust`：`view`、`revoke`、`sign` 子命令 [docker/cli#472](https://github.com/docker/cli/pull/472)
...
* 使用每节点、每网络 LB 端点在 Windows 上启用 ILB/ELB [moby/moby#34674](https://github.com/moby/moby/pull/34674)
* Overlay 修复瞬时 IP 重用问题 [docker/libnetwork#1935](https://github.com/docker/libnetwork/pull/1935)
* 序列化 bitseq 分配 [docker/libnetwork#1788](https://github.com/docker/libnetwork/pull/1788)
- 在链存在性检查时禁用主机名查找 [docker/libnetwork#1974](https://github.com/docker/libnetwork/pull/1974)

### 运行时

* LCOW：通过在拆除前获取日志来添加 UVM 可调试性 [moby/moby#34846](https://github.com/moby/moby/pull/34846)
* LCOW：为绑定挂载准备工作 [moby/moby#34258](https://github.com/moby/moby/pull/34258)
* LCOW：支持 docker cp，构建时支持 ADD/COPY [moby/moby#34252](https://github.com/moby/moby/pull/34252)
...
* 如果在 worker 节点上执行 `docker swarm init --force-new-cluster`，产生错误 [moby/moby#34881](https://github.com/moby/moby/pull/34881)
+ 在 swarm 服务中添加对 `.Node.Hostname` 模板的支持 [moby/moby#34686](https://github.com/moby/moby/pull/34686)
* 将发送快照的 gRPC 请求超时增加到 20 秒 [docker/swarmkit#2391](https://github.com/docker/swarmkit/pull/2391)
- 如果 logdriver 设置为 `none` 则不过滤节点 [docker/swarmkit#2396](https://github.com/docker/swarmkit/pull/2396)
