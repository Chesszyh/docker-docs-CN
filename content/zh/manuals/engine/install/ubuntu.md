---
description: 使用 Ubuntu 上的 Docker Engine 快速启动您的客户端服务器应用程序。本指南详细介绍了前提条件以及在 Ubuntu 上安装 Docker Engine 的多种方法。
keywords: docker 安装脚本, ubuntu docker 服务器, ubuntu server docker, 在 ubuntu 上安装 docker engine, 在 ubuntu 服务器上安装 docker, ubuntu 22.04 docker-ce, 在 ubuntu 上安装 docker engine, ubuntu 安装 docker ce, ubuntu 安装 docker engine
...
要在 Ubuntu 上开始使用 Docker Engine，请确保您[满足前提条件](#prerequisites)，然后按照[安装步骤](#installation-methods)进行操作。
...
适用于 Ubuntu 的 Docker Engine 兼容 x86_64 (或 amd64)、armhf、arm64、s390x 和 ppc64le (ppc64el) 架构。

> [!NOTE]
> 
> 官方不提供对 Ubuntu 衍生发行版 (如 Linux Mint) 的安装支持 (尽管它可能可以工作)。
