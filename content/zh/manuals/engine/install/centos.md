...
keywords: 要求, dnf, yum, 安装, centos, install, 卸载, docker engine, 升级, 更新
...
要在 CentOS 上开始使用 Docker Engine，请确保您[满足前提条件](#prerequisites)，然后按照[安装步骤](#installation-methods)进行操作。

## 前提条件

### 操作系统要求

要安装 Docker Engine，您需要以下 CentOS 版本之一的受维护版本：

- CentOS 9 (stream)

必须启用 `centos-extras` 仓库。该仓库默认启用。如果您已禁用它，则需要重新启用它。
...
安装 `dnf-plugins-core` 软件包 (它提供了管理 DNF 仓库的命令) 并设置仓库。
...
1. 安装 Docker 软件包。
   ...
   返回的列表取决于启用了哪些仓库，并且特定于您的 CentOS 版本 (在此示例中由 `.el9` 后缀指示)。

   通过其完全限定的软件包名称安装特定版本，该名称由软件包名称 (`docker-ce`) 加上版本字符串 (第 2 列) 组成，中间用连字符 (`-`) 分隔。例如，`docker-ce-3:{{% param "docker_ce_version" %}}-1.el9`。
...
1. 前往 [{{% param "download-url-base" %}}/]({{% param "download-url-base" %}}/) 并选择您的 CentOS 版本。然后浏览到 `x86_64/stable/Packages/` 并下载您要安装的 Docker 版本的 `.rpm` 文件。
...
要升级 Docker Engine，请下载较新的软件包文件并重复[安装过程](#install-from-a-package)，使用 `dnf upgrade` 代替 `dnf install`，并指向新文件。
