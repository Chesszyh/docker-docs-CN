---
description: 了解如何在 CentOS 上安装 Docker Engine。这些说明涵盖
  不同的安装方法、如何卸载以及后续步骤。
...
要在 CentOS 上开始使用 Docker Engine，请确保您
...
必须启用 `centos-extras` 仓库。此仓库默认已启用。如果您已禁用它，则需要重新启用它。
...
1. 前往 [{{% param "download-url-base" %}}/]({{% param "download-url-base" %}}/)
   并选择您的 CentOS 版本。然后浏览到 `x86_64/stable/Packages/`
