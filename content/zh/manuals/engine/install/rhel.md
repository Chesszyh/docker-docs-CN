--- 
description: 了解如何在 RHEL 上安装 Docker Engine。这些说明涵盖了不同的安装方法、如何卸载以及后续步骤。
keywords: 要求, dnf, 安装, rhel, rpm, install, 安装 docker engine, 卸载, 升级, 更新
...
要在 RHEL 上开始使用 Docker Engine，请确保您[满足前提条件](#prerequisites)，然后按照[安装步骤](#installation-methods)进行操作。

## 前提条件

### 操作系统要求

要安装 Docker Engine，您需要以下 RHEL 版本之一的受维护版本：
...
1. 安装 Docker 软件包。
   ...
   返回的列表取决于启用了哪些仓库，并且特定于您的 RHEL 版本 (在此示例中由 `.el9` 后缀指示)。
...
4. 下载以下适用于 Docker Engine、CLI、containerd 和 Docker Compose 软件包的 `rpm` 文件：
...
5. 安装 Docker Engine，将以下路径更改为您下载软件包的路径。
