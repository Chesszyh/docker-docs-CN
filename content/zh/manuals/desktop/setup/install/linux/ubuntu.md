---
description: 了解如何在 Ubuntu 上安装、启动和升级 Docker Desktop。本快速指南涵盖了前提条件、安装方法等内容。
keywords: 安装 docker ubuntu, ubuntu 安装 docker, 在 ubuntu 上安装 docker, docker 安装 ubuntu, 如何在 ubuntu 上安装 docker, ubuntu docker 安装, docker 在 ubuntu 上的安装, docker ubuntu 安装, docker 正在安装 ubuntu, 在 ubuntu 上安装 docker, ubuntu 版 docker desktop
...
{{< summary-bar feature_name="Docker Desktop Ubuntu" >}}
...
- 拥有搭载 x86-64 架构且运行 Ubuntu 22.04、24.04 或最新非 LTS 版本的系统。
...
1. 设置 Docker 的软件包存储库。
   请参阅 [使用 apt 存储库安装](/manuals/engine/install/ubuntu.md#install-using-the-repository) 的第一步。
...
- 按照 [Docker 工作坊](/get-started/workshop/_index.md) 学习如何构建镜像并将其作为容器化应用程序运行。
