...
Bake 是 Docker Buildx 的一项功能，它允许您使用声明式文件来定义构建配置，而不是指定复杂的 CLI 表达式。它还允许您通过单次调用并发运行多个构建。
...
`group` 块定义了一组可以并发构建的目标。每个 `target` 块定义了一个具有自己配置的构建目标，例如构建上下文、Dockerfile 和标签。
...
这将执行 `default` 组，该组会并发构建 `frontend` 和 `backend` 目标。
