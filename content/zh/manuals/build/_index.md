...
title: Docker Build
weight: 20
description: 获取 Docker Build 概览，以打包和捆绑您的代码并将其发布到任何地方
...
- title: 构建缓存
  description: 避免不必要地重复昂贵的操作，例如包安装。
  icon: cycle
  link: /build/cache/
- title: Bake
...
Docker Build 是 Docker 引擎最常用的功能之一。无论何时创建镜像，您都在使用 Docker Build。构建是软件开发生命周期的关键部分，允许您打包和捆绑代码并将其发布到任何地方。

Docker Build 不仅仅是一个用于构建镜像的命令，也不仅仅是关于打包代码。它是一个完整的工具和功能生态系统，不仅支持常见的工作流任务，还为更复杂和高级的场景提供支持。
