...
在 [第 1 部分](./02_our_app.md) 中，您容器化了一个待办事项应用程序。在这部分中，您将更新应用程序和镜像。您还将学习如何停止和删除容器。

## 更新源代码

在以下步骤中，您将把没有待办事项时的“空文本”更改为“You have no todo items yet! Add one above!”。
...
要删除容器，您首先需要停止它。一旦停止，您就可以将其删除。您可以使用 CLI 或 Docker Desktop 的图形界面删除旧容器。选择您最熟悉的选项。
...
2. 使用 `docker stop` 命令停止容器。将 `<the-container-id>` 替换为来自 `docker ps` 的 ID。
...
> [!NOTE]
>
> 您可以通过向 `docker rm` 命令添加 `force` 标志，在一个命令中停止并删除容器。例如：`docker rm -f <the-container-id>`
...
2. 为您要删除的容器选择 **Actions**（操作）列下的垃圾桶图标。
...
2. 在 [http://localhost:3000](http://localhost:3000) 上刷新浏览器，您应该会看到更新后的帮助文本。
