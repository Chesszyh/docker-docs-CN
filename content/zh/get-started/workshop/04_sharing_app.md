...
keywords: get started, setup, orientation, quickstart, intro, concepts, containers,
  docker desktop, docker hub, sharing, 入门, 设置, 概览, 快速入门, 简介, 概念, 容器, 共享
description: 共享您为示例应用程序构建的镜像，以便您可以在其他地方运行它，其他开发人员也可以使用它
aliases:
 - /get-started/part3/
...
现在您已经构建了一个镜像，您可以共享它。要共享 Docker 镜像，您必须使用 Docker 注册表。默认的注册表是 Docker Hub，这也是您使用的所有镜像的来源。
...
在下图中，您可以看到来自 Docker Hub 的 Docker 命令示例。此命令将推送到此仓库。
...
2. 要解决此问题，首先使用您的 Docker ID 登录 Docker Hub：`docker login YOUR-USER-NAME`。
3. 使用 `docker tag` 命令为 `getting-started` 镜像赋予新名称。将 `YOUR-USER-NAME` 替换为您的 Docker ID。

   ```console
   $ docker tag getting-started YOUR-USER-NAME/getting-started
   ```

4. 现在再次运行 `docker push` 命令。如果您要复制 Docker Hub 中的值，可以省略 `tagname` 部分，因为您没有向镜像名称添加标签。如果您不指定标签，Docker 将使用名为 `latest` 的标签。
...
> [!NOTE]
>
> Play with Docker 使用 amd64 平台。如果您使用的是基于 ARM 的 Mac（配备 Apple 芯片），则需要重新构建镜像以兼容 Play with Docker，并将新镜像推送到您的仓库。
>
...
4. 选择左侧边栏上的 **ADD NEW INSTANCE**（添加新实例）选项。如果您没有看到它，请将浏览器稍微调宽一点。几秒钟后，浏览器中会打开一个终端窗口。
...
5. 在终端中，启动您刚推送的应用程序。
   ...
   ```
    ...
    > [!TIP]
    >
    > 您可能已经注意到，此命令将端口映射绑定到了不同的 IP 地址。之前的 `docker run` 命令将端口发布到了主机上的 `127.0.0.1:3000`。这一次，您使用的是 `0.0.0.0`。
    >
    > 绑定到 `127.0.0.1` 仅将容器的端口公开给环回接口。然而，绑定到 `0.0.0.0` 会将容器的端口公开给主机的所有接口，使其可供外部世界访问。
    >
...
在本节中，您学习了如何通过将镜像推送到注册表来共享镜像。然后，您转到一个全新的实例，并能够运行刚刚推送的镜像。这在 CI 管道中非常常见，管道将创建镜像并将其推送到注册表，然后生产环境可以使用该镜像的最新版本。
