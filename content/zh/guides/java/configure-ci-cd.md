---
title: 为你的 Java 应用程序配置 CI/CD
linkTitle: 配置 CI/CD
weight: 40
keywords: java, CI/CD, local, development
description: 了解如何为你的 Java 应用程序配置 CI/CD
aliases:
...
完成本指南的前面部分，从 [容器化你的应用](containerize.md) 开始。你必须拥有 [GitHub](https://github.com/signup) 帐户和 [Docker](https://hub.docker.com/signup) 帐户才能完成本节。
...
1. 转到 GitHub 上的存储库，然后选择 **Actions** 选项卡。
   该项目已经有 `maven-build` 工作流程来使用 Maven 构建和测试你的 Java 应用程序。如果你愿意，你可以选择禁用此工作流程，因为你不会在本指南中使用它。你将创建一个新的、替代的工作流程来构建、测试和推送你的镜像。

2. 选择 **New workflow**（新建工作流程）。

3. 选择 **set up a workflow yourself**（自行设置工作流程）。
