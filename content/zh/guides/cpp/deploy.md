...
- 完成本指南的所有前置章节，从[容器化 C++ 应用程序](containerize.md)开始。
...
在您的 `c-plus-plus-docker` 目录中，创建一个名为
`docker-kubernetes.yml` 的文件。在 IDE 或文本编辑器中打开该文件并添加以下内容。将 `DOCKER_USERNAME/REPO_NAME` 替换为您的 Docker 用户名和您在[为 C++ 应用程序配置 CI/CD](configure-ci-cd.md)中创建的仓库名称。
...
- 一个 Deployment（部署），描述一组可扩展的相同 Pod。在这种情况下，
  您将只获得一个副本，即 Pod 的一个副本。该 Pod 在 `template` 下描述，其中只有一个容器。该容器是由[为 C++ 应用程序配置 CI/CD](configure-ci-cd.md)中 GitHub Actions 构建的镜像创建的。
- 一个 NodePort 服务，它将流量从主机的 30001 端口路由到它所路由的 Pod 内部的 8080 端口，使您可以从网络访问应用程序。

要了解更多关于 Kubernetes 对象的信息，请参阅 [Kubernetes 文档](https://kubernetes.io/docs/home/)。

## 部署并检查您的应用程序

1. 在终端中，导航到 `c-plus-plus-docker` 并将应用程序部署到 Kubernetes。
...
在本节中，您学习了如何使用 Docker Desktop 将 C++ 应用程序部署到开发机器上功能完整的 Kubernetes 环境中。
