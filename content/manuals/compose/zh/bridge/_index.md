---
description: 了解 Compose Bridge 如何将 Docker Compose 文件转换为 Kubernetes 清单以实现无缝平台转换
...
Compose Bridge 将您的 Docker Compose 配置转换为特定平台的格式——主要是 Kubernetes 清单。默认转换会生成 Kubernetes 清单和 Kustomize 覆盖层，这些都是为在启用了 Kubernetes 的 Docker Desktop 上部署而设计的。

它是一个灵活的工具，允许您利用[默认转换](usage.md)或[创建自定义转换](customize.md)以满足特定项目需求和要求。
...
Compose Bridge 使用 Go 模板为 Kubernetes 提供自己的转换，因此可以通过替换或追加您自己的模板来轻松扩展自定义。
